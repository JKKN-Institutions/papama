...
## ⚠️ Coordination note (from the 2026-06-24 vendor fix pass)

The vendor team changed settlement semantics you must account for:
- `vendors.settlement_cycle` column added (migration **m25** — `supabase/migrations/20260624010125_m25_vendor_settlement_cycle.sql`, propose-only, not yet applied). Nullable; NULL = unset.
- `lib/services/settlement.ts` `runSettlement` now **honors each vendor's `settlement_cycle`** with period windowing. The `period` arg in `app/api/admin/settlements/run/route.ts` is now a **fallback** for vendors with no chosen cycle — it is no longer a global label. Keep this in mind when you build the settlement "hold" override UI/route.
- When you implement the `held` payment-status override, note the redemption proof route now sets `locked→released` only after real proof upload (vendor m26 bucket).
